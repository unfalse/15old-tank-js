...
BattleTankGame.deps.csw.prototype.setDirectionAndAccel = function (d, accel) {
    this.d = d;
    this.inertiaDirections[d] += accel;
}

BattleTankGame.deps.csw.prototype.getDirSum = function () { 
    return this.inertiaDirections[0] + this.inertiaDirections[1] + 
        this.inertiaDirections[2] + this.inertiaDirections[3];
}
...
BattleTankGame.deps.csw.prototype.update = function (direction, isMoving) {
    ...
    if (!this.stopAccel && this.iam === this.CONST.USER) {
        for(let d = 0; d<4; d++) {
            this.move(d);
        }
    }
